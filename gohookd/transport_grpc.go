...
type GohookdTunnelServer struct {
	// Has access to the queue to listen for messages to send to connected clients
	queue HookQueue
...
func (s *GohookdTunnelServer) Tunnel(req *pb.TunnelRequest, stream pb.Gohook_TunnelServer) error {
...
func MakeGRPCServer(ctx context.Context, endpoints Endpoints, logger log.Logger) *GohookdServer {
	...
	return &GohookdServer{
		GohookdTunnelServer: GohookdTunnelServer{},
